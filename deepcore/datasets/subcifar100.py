...
class SubCIFAR100DataSet(Dataset):
    ...
    def __init__(self, root: str, transform, index, divide_step=5):
        ...
        self.step_class = SubCIFAR100DataSet.indexes[index*self.step_num:(index+1)*self.step_num]
        for c in self.step_class:
...
class SubCIFAR100TestDataSet(Dataset):
        indexes = None

        def __init__(self, root: str, transform, index, divide_step=5):
            total_class_num = 100
            if SubCIFAR100TestDataSet.indexes is None:
                if SubCIFAR100DataSet.indexes is not None:
                    SubCIFAR100TestDataSet.indexes = SubCIFAR100DataSet.indexes
                else:
                    np.random.seed(100)
                    array = np.arange(total_class_num)  # 生成 0 到 100 的数组
                    np.random.shuffle(array)
                    SubCIFAR100TestDataSet.indexes = array
            ...
            # self.targets = []
            dst_test = datasets.CIFAR100(root, train=False, download=True, transform=transform)
            n_train = len(dst_test)
            indices = np.array([], dtype=np.int64)
            self.step_class = SubCIFAR100TestDataSet.indexes[index * self.step_num:(index + 1) * self.step_num]
            for c in self.step_class:
                class_index = np.arange(n_train)[dst_test.targets == c]
                indices = np.append(indices, class_index)
            self.data = dst_test.data[indices]
            self.targets = np.array(dst_test.targets)[indices]
            self.transform = transform
...
def SubCIFAR100(data_path, index=0, divide_step=5):
    ...
    dst_test = SubCIFAR100TestDataSet(data_path, transform, index, divide_step)
...
if __name__ == '__main__':
    SubCIFAR100('/home/sample_new/data/', index=0)
    print('hello')