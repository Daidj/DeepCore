...
class ESC50DataSet(Dataset):
    def __init__(self, root: str, train: bool = True):
        ...
        self.data: Any = []
        self.targets = []
        self.classes = [x for x in range(50)]
...
def build_dataset(data_home):
    # dataset = soundata.initialize('esc50', data_home=data_home)
    ...
    for i in range(dataset_size):
        audio_path = os.path.join(audio_folder, filename[i])
        label = filetarget[i]
    ...
    test_index = []
    for c in range(50):
        indices = torch.where(all_targets == c)[0]
        train_num = round(5 * len(indices) / 6)
        train_index.append(indices[0:train_num])
        test_index.append(indices[train_num:])
    ...
    test_samples = all_samples[test_index]
    test_targets = all_targets[test_index]

    torch.save(train_samples, os.path.join(data_home, 'train_samples.pt'))
    torch.save(train_targets, os.path.join(data_home, 'train_labels.pt'))
    torch.save(test_samples, os.path.join(data_home, 'test_samples.pt'))
    torch.save(test_targets, os.path.join(data_home, 'test_labels.pt'))
...
def ESC50(data_path):
    ...
    mean = None
    std = None
    folder_name = os.path.join(data_path, 'ESC50')
    ...
    dst_train = ESC50DataSet(root=folder_name, train=True)
    num_classes = dst_train.num_classes
    class_names = dst_train.classes
    dst_test = ESC50DataSet(root=folder_name, train=False)
    im_size = dst_train.data[0].size(1)
    return channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test

if __name__ == '__main__':
    data_home = '/home/sample_new/data/'
    ESC50(data_home)
