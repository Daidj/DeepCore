...
def micro_search(matrix, confidence, budget: int, metric, device, random_seed=None, index=None, already_selected=None,
                    print_freq: int = 20):
    ...
    # step = 1
    similarity_redundancy_ratio = 0.75
    print('greedy init')
    while init_num > 0:
    ...
    # min_dis = 1 / (min_dis + 1e-3)
    # min_dis = (min_dis - min_dis.min() + 1e-3) / (min_dis.max() - min_dis.min())
    # uncertainty = confidence[unselected_tensor]
    #
    # scores = uncertainty + similarity_redundancy_ratio * min_dis
    ...
    def select(self, **kwargs):
        ...
        if self.balance:
            ...
            for c in range(self.args.num_classes):
                ...
                selection_result = np.append(selection_result, micro_search(features_matrix, confidence,
                                                                               budget=budget,
                                                                               metric=self.metric,
